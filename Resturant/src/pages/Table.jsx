import React, { useState, useEffect } from 'react';
import TableUnit from './TableUnit';
import './Table.css';

const Table = () => {
  const [tables, setTables] = useState([]);
  const [showModal, setShowModal] = useState(false);
  const [newTable, setNewTable] = useState({
    tableName: '',
    tableNumber: 0,
    tableSize: 2,
  });
  const [isCreating, setIsCreating] = useState(false);
  const [error, setError] = useState('');

  // Fetch all tables
  const fetchTables = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/tables`);
      if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);

      const data = await response.json();
      if (data.success) {
        const tablesList = data.data.map((t, i) => ({
          ...t,
          fakeName: i + 1,
        }));

        setTables(tablesList);

        // Calculate next available table number
        const nextNumber =
          tablesList.length > 0
            ? Math.max(...tablesList.map((t) => t.tableNumber || 0)) + 1
            : 1;

        setNewTable({
          tableName: `Table ${nextNumber}`,
          tableNumber: nextNumber,
          tableSize: 2,
        });
      } else {
        throw new Error(data.message || 'Failed to load tables');
      }
    } catch (err) {
      console.error('Error fetching tables:', err);
      setError('Failed to load tables: ' + err.message);
    }
  };

  // Delete table
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/tables/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);

      const data = await response.json();
      if (data.success) fetchTables();
      else throw new Error(data.message || 'Failed to delete table');
    } catch (err) {
      console.error('Error deleting table:', err);
      setError('Failed to delete table: ' + err.message);
    }
  };

  // Create new table
  const handleCreate = async () => {
    if (isCreating) return;
    if (!newTable.tableName.trim()) {
      setError('Please enter a table name');
      return;
    }

    setIsCreating(true);
    setError('');

    try {
      const tableData = {
        tableName: newTable.tableName.trim(),
        tableNumber: parseInt(newTable.tableNumber),
        tableSize: parseInt(newTable.tableSize),
      };

      const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/tables`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(tableData),
      });

      if (!response.ok) {
        const errRes = await response.json().catch(() => ({}));
        throw new Error(errRes.message || `HTTP error! status: ${response.status}`);
      }

      const data = await response.json();
      if (data.success) {
        setShowModal(false);
        setNewTable({ tableName: '', tableNumber: 0, tableSize: 2 });
        await fetchTables();
      } else {
        throw new Error(data.message || 'Failed to create table');
      }
    } catch (err) {
      console.error('Error creating table:', err);
      setError('Error creating table: ' + err.message);
    } finally {
      setIsCreating(false);
    }
  };

  useEffect(() => {
    fetchTables();
  }, []);

  const openModal = () => {
    const nextNumber =
      tables.length > 0 ? Math.max(...tables.map((t) => t.tableNumber || 0)) + 1 : 1;
    setNewTable({
      tableName: `Table ${nextNumber}`,
      tableNumber: nextNumber,
      tableSize: 2,
    });
    setShowModal(true);
    setError('');
  };

  const closeModal = () => {
    setShowModal(false);
    setIsCreating(false);
    setError('');
  };

  const handleInputChange = (e) => {
    const { name, value } = e.target;
    setNewTable((prev) => ({
      ...prev,
      [name]: value,
    }));
    if (error) setError('');
  };

  const handleKeyPress = (e) => {
    if (e.key === 'Enter' && !isCreating) handleCreate();
  };

  const renderTableRows = () => {
    const rows = [];
    for (let i = 0; i < tables.length; i += 7) {
      const rowTables = tables.slice(i, i + 7);
      rows.push(
        <div className="table-row" key={i}>
          {rowTables.map((table) => (
            <TableUnit key={table._id} table={table} onDelete={handleDelete} />
          ))}
        </div>
      );
    }
    return rows;
  };

  return (
    <div className="table-container">
      <h2>Tables</h2>
      {error && (
        <div className="error-banner">
          {error}
          <button onClick={() => setError('')} className="error-close">
            ×
          </button>
        </div>
      )}

      {renderTableRows()}
      <button className="add-table-btn" onClick={openModal}>
        +
      </button>

      {showModal && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h2>Create New Table</h2>

            {error && <div className="error-message">{error}</div>}

            <div className="form-group">
              <label>Table Number *</label>
              <input
                type="number"
                // name="tableNumber"
                value={newTable.tableNumber}
                onChange={handleInputChange}
                placeholder="Enter table number"
                disabled={isCreating}
                min="1"
              />
            </div>

            <div className="form-group">
              <label>Table Name *</label>
              <input
                type="text"
                name="tableName"
                value={newTable.tableName}
                onChange={handleInputChange}
                onKeyPress={handleKeyPress}
                placeholder="Enter table name"
                disabled={isCreating}
              />
            </div>

            <div className="form-group">
              <label>Table Size (Chairs)</label>
              <select
                name="tableSize"
                value={newTable.tableSize}
                onChange={handleInputChange}
                disabled={isCreating}
              >
                <option value={2}>2</option>
                <option value={4}>4</option>
                <option value={6}>6</option>
                <option value={8}>8</option>
              </select>
            </div>

            <div className="modal-actions">
              <button
                className="create-btn"
                onClick={handleCreate}
                disabled={isCreating || !newTable.tableName.trim() || !newTable.tableNumber}
              >
                {isCreating ? 'Creating...' : 'Create'}
              </button>
              <button className="cancel-btn" onClick={closeModal} disabled={isCreating}>
                Cancel
              </button>
            </div>
          </div>
        </div>
      )}
    </div>
  );
};

export default Table;
